...
// GetOne provides a mock function with given fields: token, commentID
func (_m *Service) GetOne(token *jwt.Token, commentID uint) (models.Comment, error) {
	ret := _m.Called(token, commentID)

	var r0 models.Comment
	var r1 error
	if rf, ok := ret.Get(0).(func(*jwt.Token, uint) (models.Comment, error)); ok {
		return rf(token, commentID)
	}
	if rf, ok := ret.Get(0).(func(*jwt.Token, uint) models.Comment); ok {
