...
func TestRegister(t *testing.T) {
	...
	var userData = user.User{Username: "john_doe", Password: "password123"}
	var repoData = user.User{Username: "john_doe", Password: "some string"}
	var falseUserData = user.User{Username: "", Password: ""}
...
func TestDapatUser(t *testing.T) {
	...
	t.Run("Error Case - Repository Failure", func(t *testing.T) {
		repo.On("GetUserByUsername", username).Return(user.User{}, errors.New("database error")).Once()
...
func (mr *MockRepository) Insert(newUser user.User) (user.User, error) {
	return user.User{ID: uint(1), Username: "jerry"}, nil
}
func (mr *MockRepository) Login(Username string) (user.User, error) {
...
func (fmr *FalseMockRepository) Login(Username string) (user.User, error) {
	return user.User{}, nil
}
